...
class NWDoorRegionAgent:
        ...
        while(self.start_check_flag):
            ...
            if(self.detail_check_flag):
                ...
                if(door_is_open):
                    ...
                    self.check_door_distance(self.doors, self.door_index)
                    self.robot.mo_access_control.set_flip_loop_flag(False)
    ...
    def configure_doors(self):
        ...
        door_ids = self.robot.nwdb.get_value_with_conditions('nw.event.region', 'ID', {'layout_id': self.robot.layout_nw_id, 'is_door': 1})
        
        self.doors.clear()
        for door_id in door_ids:
            # Fetch the remaining properties from the database based on the door ID
            layout_id = self.robot.nwdb.get_single_value('nw.event.region', 'layout_id', 'ID', door_id)
            name = self.robot.nwdb.get_single_value('nw.event.region', 'name', 'ID', door_id)
            polygon = self.robot.nwdb.get_single_value('nw.event.region', 'polygon', 'ID', door_id)
            
            # Create a Door object and append it to the list
            # door = Door(layout_id, name, layout_pos_x, layout_pos_y, layout_pos_heading, is_closed=True)
            door = DoorRegion(name, polygon, layout_id, is_closed=True)
            self.doors.append(door)
        return self.doors

    def check_door_distance(self, doors, index):
        
        while(True):
            
            inside = self.region_handler.checkRegion(self.robot.status.layoutPose.x,self.robot.status.layoutPose.y,
                                                     self.region_handler.parseRegionData(doors[index].polygon))
