...
class Robot:
    ...
    def rv_charging_stop(self, task_json, status_callback):
        if(self.rvapi.get_battery_state().powerSupplyStatus == "DISCHARGING"): return True
        try:
            # # update robot_status
            self.is_charging = False
    ...
    def thread_li_liftin_audio(self, cur_floor_int, target_floor_int, task_json):
        '''
        e.g. 
        cur_floor_int = 6
        target_floor_int = 7
    ...
    def li_liftin_audio(self, task_json, status_callback):
        try:
            self.lift_task_json = task_json
            self.goto(task_json, status_callback)
            cur_floor_int = int(task_json['parameters']['current_floor'])
            target_floor_int = int(task_json['parameters']['target_floor'])
            threading.Thread(target=self.thread_li_liftin_audio, args=(cur_floor_int,target_floor_int, task_json)).start()
            # 0, 7 or 7,0
            return True
        except:
            return False

    def thread_li_liftout_audio(self, final_floor_int, target_floor_int,task_json,status_callback):
        '''
        e.g. 
        final_floor_int = 0
        target_floor_int = 7
    ...
    def thread_li_liftin_levelling(self, cur_floor_int,task_json,status_callback):
        '''
        e.g. 
        cur_floor_int:0
        '''
    ...
    def thread_li_liftout_levelling(self, target_floor_int, task_json, status_callback):
        '''
        e.g.
        target_floor_int = 7
        '''
        ...
        for idx, floor_no in enumerate(floor_list): # e.g 0-7
            ...
            ## check levelling
            self.inspect_lift_levelling(task_json)
    ...
    def delivery_mission_publisher(self):
        ...
        # charging off
        self.missionpub.constrcut_charging_off(6,180)
