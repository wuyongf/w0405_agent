...
class rpc:
    ...
    def __tab_crc_16(self, data, size):  # private
        crc = 0xFFFF
        for i in range(size):
            crc = self.__crc_16_table[(
                (crc >> 8) ^ data[i]) & 0xff] ^ (crc << 8)
        return crc & 0xFFFF
    ...
    def _get_packet(self, magic_value, payload_buf_tuple, timeout):  # private
        ...
        if packet is not None:
            ...
            if magic == magic_value and crc == self.__crc_16(
                    packet,
                    len(packet) - 2):
    ...
    def _set_packet(self, magic_value, payload=bytes()):  # private
        ...
        new_payload[-2:] = struct.pack(
            "<H", self.__crc_16(new_payload,
                                len(payload) + 2))
        return new_payload
    ...
    def stream_reader(self,
                      call_back,
                      queue_depth=1,
                      read_timeout_ms=5000):  # public
        try:
            self._stream_put_bytes(
                self._set_packet(0xEDF6, struct.pack("<I", queue_depth)), 1000)
        except OSError:
            return
        tx_lfsr = 255
        while True:
            ...
            magic = packet[0] | (packet[1] << 8)
            crc = packet[-2] | (packet[-1] << 8)
            if magic != 0x542E and crc != self.__crc_16(
                    packet,
                    len(packet) - 2):
                return
            data = self._stream_get_bytes(
                bytearray(struct.unpack("<I", packet[2:-2])[0]),
                read_timeout_ms)
    ...
    def stream_writer(self, call_back, write_timeout_ms=5000):  # public
        ...
        while True:
            ...
            if credits > 0:
                ...
                try:
                    self._stream_put_bytes(
                        self._set_packet(0x542E, struct.pack("<I", len(data))),
                        1000)
                except OSError:
                    return
                try:
...
class rpc_master(rpc):
    ...
    def __init__(self):  # private
        ...
        self.__out_result_header_ack = self._set_packet(
            self._RESULT_HEADER_PACKET_MAGIC)
        self.__in_result_header_buf = self._get_packet_pre_alloc(4)
        self.__out_result_data_ack = self._set_packet(
    ...
    def __put_command(self, command, data, timeout):  # private
        ...
        out_header = self._set_packet(self._COMMAND_HEADER_PACKET_MAGIC,
                                      struct.pack("<II", command, len(data)))
        ...
        while (int(time.time() * 1000) - start) < timeout:
            ...
            self._zero(self.__in_command_data_buf[0],
                       len(self.__in_command_data_buf[0]))
            self._flush()
            self.put_bytes(out_header, self._put_short_timeout)
            if self._get_packet(self._COMMAND_HEADER_PACKET_MAGIC,
                                self.__in_command_header_buf,
                                self._get_short_timeout) is not None:
                self.put_bytes(out_data, self._put_long_timeout)
                if self._get_packet(self._COMMAND_DATA_PACKET_MAGIC,
    ...
    def call(self,
             ...
             data=bytes(),
             send_timeout=1000,
             recv_timeout=1000):  # public
        return self.__get_result(recv_timeout) if self.__put_command(
            self._hash(name, len(name)), data, send_timeout) else None
...
class rpc_slave(rpc):
    ...
    def __init__(self):  # private
        ...
        self.__out_command_header_ack = self._set_packet(
            self._COMMAND_HEADER_PACKET_MAGIC)
        self.__out_command_data_ack = self._set_packet(
    ...
    def loop(self, recv_timeout=1000, send_timeout=1000):  # public
        while True:
            ...
            if command is not None:
                ...
                if self.__put_result(
                        cb(data) if cb is not None else bytes(),
...
class rpc_uart_master(rpc_master):
    ...
    def __get_bytes(self, buff):
        ...
        l = len(buff)
        while l:
            data = self.__ser.read(min(l,
                                       1024))  # Starts a new timeout per call.
            data_len = len(data)
    ...
    def get_bytes(self, buff, timeout_ms):  # protected
        if int(self.__ser.timeout * 100) != 1:
            self.__ser.timeout = 0.01  # Changing this causes control transfers.
    ...
    def _stream_get_bytes(self, buff, timeout_ms):  # protected
        if int(self.__ser.timeout) != 1:
            self.__ser.timeout = 1  # Changing this causes control transfers.
...
class rpc_network_master(rpc_master):
    ...
    def __valid_udp_socket(self):  # private
        if self.__udp__socket is None:
            try:
                self.__udp__socket = socket.socket(socket.AF_INET,
                                                   socket.SOCK_DGRAM)
    ...
    def get_bytes(self, buff, timeout_ms):  # protected
        ...
        if l <= self._udp_limit:
            if self.__valid_udp_socket():
                try:
                    self.__udp__socket.settimeout(self._get_short_timeout *
                                                  0.001 * self._timeout_scale)
                    while l:
    ...
    def put_bytes(self, data, timeout_ms):  # protected
        ...
        if l <= self._udp_limit:
            if self.__valid_udp_socket():
                try:
                    self.__udp__socket.settimeout(self._put_short_timeout *
                                                  0.001 * self._timeout_scale)
                    while l:
    ...
    def _stream_put_bytes(self, data, timeout_ms):  # protected
        ...
        if self.__valid_tcp_socket():
            try:
                self.__tcp__socket.settimeout(timeout_ms * 0.001)
                while l:
                    data_len = self.__tcp__socket.send(data[i:i +
                                                            min(l, 1400)])
                    if not data_len: break
                    i += data_len
                    l -= data_len
...
class rpc_network_slave(rpc_slave):
    ...
    def __valid_tcp_socket(self):  # private
        if self.__tcp__socket is None:
            try:
                self.__tcp__socket = socket.socket(socket.AF_INET,
                                                   socket.SOCK_STREAM)
...
def get_can_settings(sampling_point):
    for bs1 in range(8):
        for bs2 in range(8):
            if (1 + bs1 + bs2) == 8 and (sampling_point * 10) == ((
                (1 + bs1) * 1000) // (1 + bs1 + bs2)):
                return (bs1, bs2)
